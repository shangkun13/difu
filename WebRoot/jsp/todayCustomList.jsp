...
<tr>
    		...
    		<%} %>
					...
					<td>
						共<%=count %>条        第<input style="width: 30" type="text" id="cpage" name="cpage" value=<%=cpage %> />页，共 <%=pageCount %>页    每页显示<input style="width: 30" type="text" id="pageSize" name="pageSize" value=<%=pageSize %> />条&nbsp;&nbsp;<input type="button" value="go" onclick="go();"/>
					</td>
					<td>
					<% if(cpage!=pageCount) {%>
...
function nextPage(){
	window.location.href="queryAllToday.do?cpage="+ (parseInt($("#cpage").val())+1)+"&pageSize="+$("#pageSize").val();
}
function lastPage(){
	window.location.href="queryAllToday.do?cpage="+ (parseInt($("#cpage").val())-1)+"&pageSize="+$("#pageSize").val();
}
function go(){
	window.location.href="queryAllToday.do?cpage="+$("#cpage").val()+"&pageSize="+$("#pageSize").val();
}
